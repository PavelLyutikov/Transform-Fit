//
//  ChosenStretchingTableViewController.swift
//  Transform
//
...
class ChosenStretchingTableViewController: UIViewController {
    ...
    var currentIndexStretching = UserDefaults.standard.integer(forKey: "currentIndexStretching")
    var openNextStretching = UserDefaults.standard.bool(forKey: "openNextStretching")
    var openBackStretching = UserDefaults.standard.bool(forKey: "openBackStretching")
...
//MARK: - viewDidLoad
    override func viewDidLoad() {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.3, repeats: false) { (timer) in
            if self.openNextStretching == true {
            UserDefaults.standard.set(false, forKey: "openNextStretching")
            self.nextWorkout()
            }
            if self.openBackStretching == true {
            UserDefaults.standard.set(false, forKey: "openBackStretching")
            self.backWorkout()
            }
        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (!chosenExercisesStretching!.isEmpty) {
...
//MARK: - spawnImageBackgroundTransition
    func spawnImageBackgroundTransition() {
        let indexPath = IndexPath(item: currentIndexStretching + 1, section: 0)
        
        let countList = IndexPath(item: chosenExercisesStretching!.count - 1, section: 0)
        
        let countListBack = IndexPath(item: 2, section: 0)
        
        if indexPath <= countList {
            if openNextStretching == true {
                imageBackgroundTransition.isHidden = false
            }
        }
        if indexPath >= countListBack {
            if openBackStretching == true {
                imageBackgroundTransition.isHidden = false
            }
        }
    }
...
//MARK: - NextWorkout
     func nextWorkout() {
        ...
        if indexPath <= countList {
        let chosenExerciseVideo = chosenExercisesStretching?[indexPath.row]
        
        UserDefaults.standard.set(indexPath.row, forKey: "currentIndexStretching")

        selectedTransform = warmUp.first(where: { $0.video == chosenExerciseVideo })
        
        if selectedTransform == nil {
        selectedTransform = flexibleBack.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = sideSplits.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = longTwine.first(where: { $0.video == chosenExerciseVideo })
        }
        
        UserDefaults.standard.set(true, forKey: "isOpenStretching")
        
        
        self.performSegue(withIdentifier: "chosenStretchingSegue", sender: nil)
        } else {
...
//MARK: - SetUpTableViewFooter
    private func setUpTableViewFooter() {
        ...
        let wasEmpty = chosenExercisesStretching?.isEmpty
    ...
    func showClearAlert() {
        ...
        alert.addAction(UIAlertAction(title: "Очистить".localized(), style: .destructive, handler: { (clearAction) in
            ...
            self.chosenExercisesStretching!.removeAll()
...
//MARK: - Prepare
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let controller = segue.destination as? ChosenStretchingViewController else { return }
...
//MARK: - TableView
extension ChosenStretchingTableViewController: UITableViewDelegate, UITableViewDataSource {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let chosenExercises = chosenExercisesStretching else { return 0 }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var chosenExercise = warmUp.first(where: { $0.video == chosenExerciseVideo })
        
        if chosenExercise == nil {
            chosenExercise = flexibleBack.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = sideSplits.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = longTwine.first(where: { $0.video == chosenExerciseVideo })
        }
        
        cell.stretchingImageView.image = chosenExercise?.image
        cell.stretchingLabel1.text = chosenExercise?.text1
        cell.stretchingLabel1.font = UIFont.systemFont(ofSize: fontSizeLb1)
        cell.stretchingLabel2.text = chosenExercise?.text2
        cell.stretchingLabel2.font = UIFont.systemFont(ofSize: fontSize)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "chosenStretchingSegue", sender: nil)
    }
//SwipeCell
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let selectedExercise = chosenExercisesStretching?[indexPath.row]
        
        let chosenAction = UIContextualAction(style: .destructive, title: "", handler: { (action, view, boolValue) in
            guard let chosenIndex = self.chosenExercisesStretching?.firstIndex(where: { $0 == selectedExercise }) else { return }
                
            self.chosenExercisesStretching?.remove(at: chosenIndex)
            
            
            let indexPathCount = IndexPath(item: self.chosenExercisesStretching!.count, section: 0)
            let countList = IndexPath(item: 0, section: 0)
...
extension ChosenStretchingTableViewController {
    ...
    override func positionChanged(currentIndex: IndexPath, newIndex: IndexPath) {
        ...
        chosenExercisesStretching?.swapAt(currentIndex.row, newIndex.row)
    }
}

//MARK: - ExtensionIronSource
extension ChosenStretchingTableViewController: ISBannerDelegate, ISImpressionDataDelegate, ISRewardedVideoDelegate {
