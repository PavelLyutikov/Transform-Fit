//
//  ChosenHomeWorkoutViewController.swift
//  Transform
//
//  Created by Pavel Lyutikov on 12.05.2021.
//

import UIKit

class ChosenHomeWorkoutTableViewController: UIViewController {
    ...
    var currentWorkoutIndex = UserDefaults.standard.integer(forKey: "currentWorkoutIndex")
    var openNextHomeWorkout = UserDefaults.standard.bool(forKey: "openNextHomeWorkout")
    var openBackHomeWorkout = UserDefaults.standard.bool(forKey: "openBackHomeWorkout")
    ...
    var selectedTransform: HomeWorkout?
...
//        chosenExersicesTableView.layer.zPosition = 5
        ...
        Timer.scheduledTimer(withTimeInterval: 0.3, repeats: false) { (timer) in
            if self.openNextHomeWorkout == true {
            UserDefaults.standard.set(false, forKey: "openNextHomeWorkout")
            self.nextWorkout()
            }
            if self.openBackHomeWorkout == true {
            UserDefaults.standard.set(false, forKey: "openBackHomeWorkout")
            self.backWorkout()
            }
        }
...
//MARK: - spawnImageBackgroundTransition
    func spawnImageBackgroundTransition() {
        let indexPath = IndexPath(item: currentWorkoutIndex + 1, section: 0)
        
        let countList = IndexPath(item: chosenExercises!.count - 1, section: 0)
        
        let countListBack = IndexPath(item: 2, section: 0)
        
        if indexPath <= countList {
            if openNextHomeWorkout == true {
                imageBackgroundTransition.isHidden = false
            }
        }
        if indexPath >= countListBack {
            if openBackHomeWorkout == true {
                imageBackgroundTransition.isHidden = false
            }
        }
    }
//MARK: - NextWorkout
     func nextWorkout() {
        ...
        let chosenExerciseVideo = chosenExercises?[indexPath.row]
        
        UserDefaults.standard.set(indexPath.row, forKey: "currentWorkoutIndex")

        selectedTransform = breats.first(where: { $0.video == chosenExerciseVideo })
        
        if selectedTransform == nil {
        selectedTransform = shoulder.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = hand.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = cor.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = foot.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = back.first(where: { $0.video == chosenExerciseVideo })
        }
        if selectedTransform == nil {
        selectedTransform = glutes.first(where: { $0.video == chosenExerciseVideo })
        }
            
        UserDefaults.standard.set(true, forKey: "isOpenHomeWorkout")
        
        self.performSegue(withIdentifier: "chosenHomeWorkoutSegue", sender: nil)
        } else {
...
//MARK: - SetUpTableViewFooter
    private func setUpTableViewFooter() {
        ...
        let wasEmpty = chosenExercises?.isEmpty
    ...
    func showClearAlert() {
        ...
        alert.addAction(UIAlertAction(title: "Очистить".localized(), style: .destructive, handler: { (clearAction) in
            ...
            self.chosenExercises!.removeAll()
...
//MARK: - EmptyTableViewLabel
    lazy var emptyTableViewLabel: UILabel = {
       ...
       let lbl = UILabel()
        lbl.text = "Ваш список упражнений пуст, добавьте упражнения нужной Вам мышечной группы, чтобы начать тренировку".localized()
...
//MARK: - Prepare
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let controller = segue.destination as? ChosenHomeWorkoutViewController else { return }
...
//MARK: - TableView
extension ChosenHomeWorkoutTableViewController: UITableViewDelegate, UITableViewDataSource {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let chosenExercises = chosenExercises else { return 0 }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var chosenExercise = breats.first(where: { $0.video == chosenExerciseVideo })
        
        if chosenExercise == nil {
            chosenExercise = shoulder.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = hand.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = cor.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = foot.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = back.first(where: { $0.video == chosenExerciseVideo })
        }
        
        if chosenExercise == nil {
            chosenExercise = glutes.first(where: { $0.video == chosenExerciseVideo })
        }
        
        cell.homeWorkoutImageView.image = chosenExercise?.image
        cell.homeWorkoutLabel1.text = chosenExercise?.text1
        cell.homeWorkoutLabel1.font = UIFont.systemFont(ofSize: fontSizeLb1)
        cell.homeWorkoutLabel2.text = chosenExercise?.text2
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "chosenHomeWorkoutSegue", sender: nil)
    }
//SwipeCell
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let selectedExercise = chosenExercises?[indexPath.row]
        
        let chosenAction = UIContextualAction(style: .destructive, title: "", handler: { (action, view, boolValue) in
            guard let chosenIndex = self.chosenExercises?.firstIndex(where: { $0 == selectedExercise }) else { return }
                
            self.chosenExercises?.remove(at: chosenIndex)
            
            
            let indexPathCount = IndexPath(item: self.chosenExercises!.count, section: 0)
            let countList = IndexPath(item: 0, section: 0)
...
extension ChosenHomeWorkoutTableViewController {
    ...
    override func positionChanged(currentIndex: IndexPath, newIndex: IndexPath) {
        ...
        chosenExercises?.swapAt(currentIndex.row, newIndex.row)
    }
}

//MARK: - ExtensionIronSource
extension ChosenHomeWorkoutTableViewController: ISBannerDelegate, ISImpressionDataDelegate, ISRewardedVideoDelegate {
