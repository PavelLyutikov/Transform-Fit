...
public class CircleMenu: UIView {
    ...
    private let radius: Double = 220 //96 радиус
    private let _2degrees: Double = 0.0174533 //раскачка после свайпа
    private let _6degrees: Double = 0.10472 //появление меню
    private let _45degrees: Double = 0.785398 //появление и исчезновение лишних иконок
    private let _55degrees: Double = 1.509931 //положение по кругу
    ...
    public func attach(to viewController: UIViewController) {
        ...
        menuButtonsView?.layer.zPosition = 101
        ...
        mainButton?.layer.zPosition = 101
    ...
    func fillButtonsPositions() {
        ...
        let endAngle = 3.9 + startAngle
        let diff = (endAngle - startAngle) / Double(maximumCountOfElements)
        for _ in 0..<visibleCountOfElements {
            let x = cos(startAngle) * radius + menuSize/2.0 - CircleMenuButton.size / 2.0 + 55 //NoAds 20
            let y = sin(startAngle) * radius + menuSize/2.0 - CircleMenuButton.size / 2.0 + 55 //NoAds 20
    ...
    private func fillLastButtonPosition() {
        let x = cos(_45degrees) * menuSize + menuSize/2.0 - CircleMenuButton.size / 2.0 - 100
        let y = sin(_45degrees) * menuSize + menuSize/2.0 - CircleMenuButton.size / 2.0 - 100
        buttonPositions.append(CGPoint(x: x, y: y))
    }
    ...
    private func createMenuButtons() {
        ...
        for _ in 0..<buttonPositions.count {
            ...
            menuButton.layer.zPosition = 101
    ...
    private func openMenu() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "openCircleMenu"), object: nil)
    }
    ...
    private func closeMenu() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "closeCircleMenu"), object: nil)
    }
    ...
    private func moveLeftAnimation() -> Promise<Void> {
        return Promise { seal in
            ...
            if indexOfZeroButtonPositionModel == 0 {
                idCircleMenu = 6
            } else if indexOfZeroButtonPositionModel == 1 {
                idCircleMenu = 7
            } else {
                idCircleMenu = indexOfZeroButtonPositionModel - 2
            }
            
            UserDefaults.standard.set(idCircleMenu, forKey: "idCircleMenu")
            
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "rotateCircleMenuLeft"), object: nil)
        }
    }
    ...
    private func moveRightAnimation() -> Promise<Void> {
        return Promise { seal in
            ...
            idCircleMenu = indexOfLastButtonPositionModel
            
            UserDefaults.standard.set(idCircleMenu, forKey: "idCircleMenu")
            
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "rotateCircleMenuRight"), object: nil)
    ...
    private func updateAppearance() {
        unfocusedBackgroundColor = UIColor.white //Цвет кнопок
        focusedBackgroundColor = UIColor(red: 60, green: 109, blue: 0) //Цвет круга при развертывании
        focusedIconColor = UIColor.white //Цвет иконки при нажатии
        unfocusedIconColor = UIColor.gray//(red: 51, green: 52, blue: 51) //Цвет значка кнопки
        blackoutColor = UIColor(red: 246, green: 246, blue: 246).withAlphaComponent(0.64) //Цвет затемнения фона при нажатии
    }
